...
import {
  useConversationCacheHook
} from "./use-conversation-cache-hook";
import {
  dataListAllUsers,
...
import {
  usePendingMessageHook
} from "./use-pending-message-hook";
...
function useHomeWorkbenchHook(): IHomeWorkbenchViewModel {
  ...
  const syncNextBeforeMessageId = useCallback((nextBeforeMessageId: number | null): void => {
  ...
  const loadActiveConversation = useCallback(async (conversationId: number): Promise<boolean> => {
    ...
    if (!hasCachedConversation && pendingConversationSummary) {
      setActiveConversation(pendingConversationSummary);
    }

    setMessageLoading(!hasCachedConversation);
    ...
    try {
      ...
      const nextBeforeMessageId = messagePage.next_before_id || nextMessages.at(0)?.id || null;

      writeConversationCache(conversationId, {
        conversation: nextConversation,
        hasMoreMessages: messagePage.has_more,
        isLoaded: true,
  ...
  const handleLoadMoreMessages = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const nextBeforeMessageId = messagePage.next_before_id || null;

      setMessages(currentMessages => {
        const nextMessages = mergeMessagePage(currentMessages, messagePage.items);
  ...
  useEffect(() => {
    if (!activeConversationId || !conversationCacheRef.current.has(activeConversationId)) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const cachedRealtimeConversation = conversationCacheRef.current.get(realtimeMessage.conversation_id);

    if (cachedRealtimeConversation?.isLoaded) {
      writeConversationCache(realtimeMessage.conversation_id, {
        messages: mergeMessage(cachedRealtimeConversation.messages, realtimeMessage)
      });
    }
